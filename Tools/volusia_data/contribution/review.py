...
def list_pending():
    ...
    conn = get_db()
    rows = conn.execute("SELECT * FROM submissions WHERE status = 'queued' ORDER BY submitted_at DESC").fetchall()
    conn.close()
    ...
    for r in rows:
        print(
            f"{r['submission_id']:<30} {r['contribution_type']:<15} {r['reviewer']:<25} {r['submitted_at'][:19]:<20} {r['author_name'] or 'Anonymous':<20}"
        )
...
def list_all(status=None):
    ...
    for r in rows:
        print(
            f"{r['submission_id']:<30} {r['contribution_type']:<15} {r['status']:<12} {r['reviewer']:<25} {r['submitted_at'][:19]:<20}"
        )
...
def main():
    ...
    list_parser.add_argument(
        "--status", choices=["queued", "approved", "rejected", "under_review"], help="Filter by status"
    )
